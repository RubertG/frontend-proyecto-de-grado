...
export const handleImageUpload = async (
  ...
  // Subida real a Supabase Storage (bucket: editor-images)
  const { supabaseBrowser } = await import("@/shared/auth/supabase-browser")
  const supabase = supabaseBrowser()
  const ext = file.name.split('.').pop() || 'bin'
  const path = `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`

  // No API de progreso nativa en supabase-js aún; simulamos callbacks incrementales básicos
  ...
  const { data: publicUrlData } = supabase.storage.from('editor-images').getPublicUrl(path)
